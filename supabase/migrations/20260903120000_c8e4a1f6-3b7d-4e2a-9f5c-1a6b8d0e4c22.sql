-- Feriados por empresa: uma data marcada como feriado passa a usar as
-- mesmas regras de horário do fim de semana (janela de horários e
-- horário-limite), tanto na oferta de horários ao cliente quanto na
-- validação do lado do servidor em criar_reserva().
...
CREATE POLICY "Tenant admin reads own feriados" ON public.feriados
  FOR SELECT USING (has_role(auth.uid(), 'super_admin') OR has_tenant_role(auth.uid(), tenant_id));

CREATE POLICY "Tenant admin inserts own feriados" ON public.feriados
  FOR INSERT WITH CHECK (has_role(auth.uid(), 'super_admin') OR has_tenant_role(auth.uid(), tenant_id));

CREATE POLICY "Tenant admin deletes own feriados" ON public.feriados
  FOR DELETE USING (has_role(auth.uid(), 'super_admin') OR has_tenant_role(auth.uid(), tenant_id));

CREATE POLICY "Tenant admin updates own feriados" ON public.feriados
...
-- Espelha bloqueios_do_tenant: leitura pública (via slug) para a página de
-- reserva conseguir saber quais datas futuras são feriados.
CREATE OR REPLACE FUNCTION public.feriados_do_tenant(_slug text)
 RETURNS TABLE(data date, motivo text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT f.data, f.motivo
...
-- criar_reserva(): data marcada como feriado passa a contar como fim de
-- semana no cálculo do horário-limite (mesma regra usada no cliente).
...
BEGIN
  ...
  -- Um feriado usa a mesma regra de horário do fim de semana.
